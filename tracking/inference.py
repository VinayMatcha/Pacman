...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        #beliefs contains the probability and distnce contains
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos, prob in self.beliefs.items():
            for newpos, newProb in self.getPositionDistribution(self.setGhostPosition(gameState, pos)).items():
                newPositions[newpos] += prob * newProb
        newPositions.normalize()
        self.beliefs = newPositions
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            if(beliefs.totalCount() == 0 or sum(beliefs.values())==0):
                self.initializeUniformly(gameState)
            else:
    ...
    def elapseTime(self, gameState):
        ...
        for i, pos in enumerate(self.particles):
            self.particles[i] =  util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, pos)))
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        ghostPosibilities = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(ghostPosibilities)
        self.particles = []
        for i in xrange(self.numParticles):
            self.particles.append(ghostPosibilities[i%len(ghostPosibilities)])
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            prob = 1
            for ghostIndex in range(len(p)):
                if noisyDistances[ghostIndex] is None:
                    if p[ghostIndex] != self.getJailPosition(ghostIndex):
                        prob = 0
                else:
                    prob *= emissionModels[ghostIndex][util.manhattanDistance(pacmanPosition, p[ghostIndex])]
            beliefs[p] += prob
